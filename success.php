...
if($code == 200 && !( curl_errno($handle)))
{
    ...
    //print_r($result);
    //echo '</pre>';
    $request_id = $result->value_a;
    //echo $amount,$request_id,$tran_date;
    $connection =  new mysqli('localhost','root','','pet_shelter');

    $sql_get_request = "select * from requests where id ='$request_id'";
    $result = $connection->query($sql_get_request);
    $result = $result->fetch_assoc();

    $paid = $result['paid'] + $amount;
    $due =  $result['due'] - $amount;

    print_r([$paid, $due]);

    $sql ="UPDATE requests SET transaction_id = '$tran_id', paid = '$paid', due = '$due',  payment_status='PAID' WHERE id='$request_id'";
